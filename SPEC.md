...
| **L3** | **efficiency preservation** *(gap-fill)* — the 2.5-D field is the **stored truth** *(for the **world**; a **body**'s truth is its original + pose — [`plans/m0-roundtrip/DESIGN.md`](plans/m0-roundtrip/DESIGN.md) §3.1)*; the tiltable/complex (patch-atlas) model is a **derived, on-demand overlay**: never persisted, never a branch in a hot-path op | the atlas is stored, or a field/proxy/collision op branches on "tiltable" |
