...
> **WHAT IS STORED HERE.** A **G**oal, **L**imit, **I**nvariant or **K** contract — stated
> **short**, with its **violation condition** and the gate that catches it.
> **NOT HERE:** the reasoning or the quotation behind an item (→ the owning plan's `DESIGN.md`) ·
> an object, map or law (→ [`ROUNDTRIP.md`](ROUNDTRIP.md)) · a status or judgement
> (→ [`ASSESSMENT.md`](ASSESSMENT.md)).
> ⚠ **An item that grows past a few lines has usually absorbed a *why*.** Move the *why* out and
> leave a pointer — `L15` reached 3 KB exactly that way on 2026-07-24, and `SPEC` asks its own
> items to be short.
...
| **G-FOXEL** ✅ | **the foxel schema is the storage limit, and a model that draws into it must SURVIVE it** — `𝕄*` is bounded by *"a model is admissible iff it draws into this schema exactly"* ([`ROUNDTRIP.md`](ROUNDTRIP.md) §2.4), which is only a limit if the schema actually stores what it claims | **GATED** (`X63`, `tests/foxel.loft`): all six slots cross a write/read exactly, and `write(rebuild(load(store(draw(read(T)))))) = T` byte-for-byte, 6/6 in-between directions. Control = `X15`'s documented lossy writer (`has_e = false`), which returns 0 of 38 edges and **breaks the trip** while still carrying the footprint. ⚠ The **palette** (`X12`/`X13`) is *not* covered and stays T4. **The schema of record is settled: moros's `Hex`** (`L13`) — hexbody's f64 height is the wider slot and the narrower one binds |
| **G-LIB** ⚠ | **hexbody SHIPS LIBRARIES, and that is the deliverable** (user, 2026-07-24): *"other projects — the editor, crawler, moros — will build on our basis. Our goal is to provide libraries for them."* ⚠ **And the consumer set is GROWING, not fixed** (same day): *"there will be similar other projects in the future: world creation, world simulation, weather, physics simulation for that — probably their own project to prevent clutter, almost all of them building in some way on our work."* So the seam must be stable enough for consumers **that do not exist yet** — and, the user's framing, *"we are pretty much a programming language: it doesn't define who can use them. Many consumers are there, some known some unknown."* **Design the seam for the unknown one**; that is what makes the split a decision rather than a chore, and it is `I-EXTEND`'s law applied to the API. So a capability is not done when a hexbody gate is green; it is done when a **consumer can depend on it** | **NOT MET, and the gap is structural**: `loft.toml` has no `[library]` entry — hexbody is packaged as the *application* kind, like crawler, while `hex_field` declares `[library] entry = …`. **No consumer references hexbody** (checked across `../crawler` and `../moros`, 2026-07-24), so the only way in is `--lib` at a sibling working tree — the exact unpinned-tree fragility that broke this suite the same day. Needs: a decision on the library split, a `[library]` entry, and cross-target parity (`loft-ship`) |
...
### Use cases — what they tell us to ALLOW, never what we ship

> **These are not goals and never were deliverables** (user, 2026-07-24): *"the demo itself is not
> hexbody's goal. All these examples are for us to know what to build, what to allow — not what to
> ship."* They earn their place by **generating requirements**: each one names a capability the
> libraries must offer, and a limit they must not cross. A use case is satisfied when a *consumer*
> could build it, not when hexbody builds it. Reasoning: [`plans/m1-moving-body/DESIGN.md`](plans/m1-moving-body/DESIGN.md) §3.

| was | the use case | what it tells us to allow |
|---|---|---|
| `G4` | **the train** — a coupled car+wagon follows a curve, wheels `= travel/radius`, wagon in line, detaches on decouple | derived motion from a joint value; a **coupling** primitive; per-part proxies. ⚠ `I10`/`I11` are its invariants and are the *consumer's* to satisfy — hexbody must make them **expressible**, not implement them |
| `G★` | **the derailment** — a coupled train off the rails tumbles, piles, settles to a deterministic rest, and a cold viewer reads it as a train wreck | **determinism** (`L7`/`I9`) is non-negotiable, because a plausible rest must replay byte-identically; and destruction must re-derive the proxy (`I7`) |
| `G✦` | **the colossus** — procedural, climbable, crushes buildings, loses a decoupled limb | the libraries must hold at **scale** and across **scale ratios** (`G3`); a body must be **decomposable** at runtime without re-authoring |

⚠ **`L1`'s *earned set* named `G★`.** With `G★` no longer a hexbody goal, the earned set is empty
here — full rigid-body dynamics is a **consumer's** call in a consumer's project. What hexbody owes
is that its libraries do not *prevent* one.
...
| **L12** | **a write is not a truncate, and a read is not a measurement** — `doc_write` **APPENDS**, so writing twice to one path leaves the second document unreachable and `doc_read` returns the **first** with `doc_code == HXF_OK` (`X64`). And `file().content()` returns **empty for non-UTF-8 bytes** (loft, both backends — `crawler/LOFT-HANDOFF.md` **H7**), so a byte length is **not an available instrument** for any binary artefact. ⚠ **It is no longer *silent*, and that word is the part that changed** — measured on `loft 2026.7.2` (2026-07-24), an 8-byte binary file gives `content().len() == 0` **plus** a warning naming the byte count and pointing at `f#format`/`f#read`. The hazard is unchanged and the rule stands; what is gone is the *invisibility* that made it a trap. Do not restate L12 as "silently" — a claim that was true of one toolchain and is checked against another is exactly the drift `X69` exists to stop. Every storage check must therefore write to a **fresh path** and measure by **content the format defines** (a cell count, a digest), never by file size | a gate reuses a path and compares stale bytes to themselves · a check reads `0 == 0 * 2` and reports "it appended" — which `tests/foxel.loft` printed before it was caught |
| **L13** | **THE VOXEL IS THE CEILING ON PERMANENT WORLD STATE** — *"I will never want to add more world information than in the limited moros voxels"* (user, 2026-07-24). moros's `Hex` is the **storage of record**, so `𝕄*` is bounded by **seven integers per hex per storey** and nothing may widen it: no extra permanent per-cell array, no named layer carrying new fabric, no parallel store that must be kept forever. Representing a moros slot differently is fine (`item` rides a named `LAYR` in HXF — that *is* `h_item`); **adding an eighth slot is not**. Consequence already binding: `h_height` is an **integer** at `HEIGHT_SCALE = 0.25` wu, so a float height is refused at the doorstep (`X67`) | a new permanent per-cell field appears anywhere · a `LAYR` section carries something moros's `Hex` has no slot for · a side table outlives its area or its time (**L14**) |
| **L15** | **THE GAME'S STATE IS NOT THE WORLD'S — hexbody offers containers, never content, and *now* is the only tense** *(user decision, 2026-07-24; the quotes and the reasoning are in [`plans/m1-moving-body/DESIGN.md`](plans/m1-moving-body/DESIGN.md) §0 and §3)*. NPCs, weather, quests and a body's **pose** are *game* state — a **fourth category** beside `L3`'s two layers and `L14`'s overlay — so `L13` never had to hold them and the world stays reconstructable from the voxels alone. hexbody may supply a **container keyed by world location and blind to its payload**, plus routines over it. It may not supply the payload's *meaning*; nor a **universal domain abstraction** — a train, a robot, an aeroplane and a terminal dock have different underlying logic and no honest abstraction spans them; nor **any future state**. Specific domain simulations are legitimate work, as **consumers in their own projects** | hexbody declares a game-domain concept instead of a container that could hold any of them (**checked**: `tests/scope.loft`, both vocabulary families) · a core geometry module `use`s an offered structure, making it mandatory rather than opt-in · **any hexbody structure holds a velocity, a trajectory, a next-tick or an intent** |
| **L14** | **a side table is AREA-LIMITED and TIME-LIMITED** — *"We can have other tables outside that for limited areas but those should also be time limited"* (user, 2026-07-24). Richer structure than the voxel is allowed, but only as a **bounded, expiring overlay** — a siege, a fire, a construction site — never as world state. This is a **third** category beside `L3`'s two: not layer 1 (permanent, global, stored) and not layer 2 (derived on demand, never persisted), but **stored, local, and mortal**. A side table with no area bound or no expiry is layer 1 by another name, and `L13` forbids it | a side table is global, or has no expiry · the world cannot be reconstructed from the voxels alone once the overlays have expired |

## I — Invariants (always hold; each is a gate whose control must fire)

| ID | invariant | control that must fire |
|---|---|---|
| **I1** | a door/window/**real gap** is an interval on the **analytic surface**, and is **stored as a material on the wall slot** — *an opening is never the ABSENCE of the wall* (`X70`): the wall continues and the opening carries its own geometry (a frame, a lintel, a ragged jamb), so material `0` is a different thing entirely and fragments the run (38 edges/0 ends → 36/2) — the edge is never removed, so a run is never fragmented, on a **straight** wall (`X48`) or a **curved** one (`X51`, the doored tower: it round-trips as one arc). **Its `t` is admissible only at an edge centre `(2i+1)/2n`** — any other value snaps and cannot be recovered (`ROUNDTRIP.md` **X48**), so the doorstep refuses it ([`ROUNDTRIP.md`](ROUNDTRIP.md) §2.4.1). The 2/3-direction edge strip is storage the feature never indexes | select edges by strip order → clear width wobbles between the zigzag and staircase sides · delete an edge instead of re-materialling it → the run fragments into 3 arcs (the doored-tower defect) |
...
| **I-POSE** | a body is `⟨original, pose, joints⟩` — original and pose **stored**, local field **derived**, and a body is **never stamped into the world field**. A robot's limb and a derailed wagon are the same case: an exact original at an arbitrary continuous pose. ⚠ **The AUTHORED body is a RIG — `⟨bones, joint limits⟩` — and never contains a pose**, and **flex is joints, not deformation** (a part that bends is *more bones*, so hexbody needs no vertex deformation, skinning weights or soft body, and `I6` survives a flexing wing unchanged) *(user decisions, 2026-07-24 — [`plans/m1-moving-body/DESIGN.md`](plans/m1-moving-body/DESIGN.md) §0, §2)* | §1.2 | stamp a free-posed body into the world lattice → it can only land on one of the 12 · **author a pose into a body definition → it is refused at the doorstep** |
...
| **I-EXTEND** | the model is **built out like a language**: new verbs, shape types and parameters are added without breaking the old ones. An existing canonical text keeps the **same bytes**, model and field across an extension; `𝕄*` grows, never shrinks. **A₂ alone is not sufficient** — every extension re-opens the census, because a newly admitted form may **collide** with an existing one under law **F** even while every text keeps its bytes; on collision the **new** form is refused, never the old ([`plans/m0-roundtrip/DESIGN.md`](plans/m0-roundtrip/DESIGN.md) §4.1) ⚠ **AND THE SAME LAW GOVERNS THE LIBRARY SEAM, one level up** (user, 2026-07-24): *"we are pretty much a programming language — it doesn't define who can use them. Many consumers are there, some known some unknown."* A published API is a published text: it grows and never shrinks, and it is designed for users you will never meet. **Consequence for `G-LIB`: the library split is decided from the OBJECTS, not from a survey of the three consumers we happen to know** — a language defines its primitives from its own semantics and lets users come | **A₂**, **F** | sort `kind` alphabetically instead of by registry position → adding one verb re-spells every existing text · admit a shape type that rasterises identically to an existing one at small size → `rebuild` cannot tell them apart |
...
| **I-QUANT** | **every continuous model parameter is quantised to what the field distinguishes** — a value off that grid is **silently snapped, not rejected**, so `fits?` must refuse it at authoring time. Measured on three independent parameters (`ROUNDTRIP.md` **X50**): line **endpoints** → hex vertices (§10.10), feature **`t`** → edge centres — **two families**, and `(2i+1)/2n` is only the tiling one; the other is inset, spanning `√3/2` of its side (**X48** as corrected by **X65**), arc **radius** → shells `3k²+m²` (**X49**) | **D**, **K-FIT** | author a value between grid points → it round-trips as a *different* value with `ρ = 0`, i.e. success is reported on a model nobody wrote (`tests/surface.loft` §6, `tests/arc.loft` §3) |
...
| **K-JOINT** | the **joint value** is the interface; downstream (proxy, collision, render) is blind to whether it came from a constraint (derived) or a track (authored). ⚠ **And the JOINT LIMIT is the doorstep** (user, 2026-07-24 — a body is bones plus *"the limitations in the joints between the bones"*): `fits?` for a body is *is this joint value inside its joint's limit*, which makes `K-FIT` apply to bodies unchanged. Unlike a material id (`X68`), a joint value is **ordinal** — so a refusal here **does** owe an offer: the nearest admissible angle, with its residual |
...
2. **Every `tests/*.loft` gate maps to a spec item** and asserts its control. A gate that
   ...
   | `joint.loft` | **I6** — ⚠ **a FORWARD gate, held RED**, and it defends nothing yet. Written before `G1` the way `rt_trip` was written before `rebuild`; `tools/run_tests.sh` runs it through `run_red`, which fails if it prints `JOINT OK` **or** if it is red for any other reason. It states the two checks `G1` must satisfy — purity by two paths, and `skid = \|r·θ − d\| = 0` with the spin-off-travel control — and deliberately does **not** assert the joint round trip, which is proposed rather than `I6`. Tripwire: any body verb declared in `src/` flips it to `JOINT BLOCKED` |
...
| `I4` `I5` `I7` `I9` `I10` `I11` | the same: proxy containment, swept volumes, re-derivation, replay, couplings | **unbuilt** — each is `G1`–`G6`'s invariant |
| `I6` | pose is a pure function of the joints | **unbuilt, but ARMED** — `tests/joint.loft` is a forward gate held red by `run_red`, stating what `G1` must satisfy. It is **still undefended**: a red gate asserts nothing, and counting it would be the `X15` mistake |
