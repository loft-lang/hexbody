...
| **I-CORNER** | **where two wall runs meet, the corner is precise** — four parts (`DESIGN.md` §10.4): (1) the boundary is **one closed loop**, no gap; (2) the corner **angle is exact**; (3) the corner edge is claimed by **exactly one** side run; (4) the **miter point** of the two fitted surfaces sits on the exact corner. **1 and 3 are gated now** (`X35`, `X36`); **2 and 4 need the fitted surface** and are a tripwire — `hexform::SURFACE_LANDED` is `false`, and flipping it at S8 fails the gate until they are written | **I3** | punch an interior cell → the boundary becomes 2 loops · drop a side → three runs cover 28 of 38 (`tests/form.loft` §14, §16) |
