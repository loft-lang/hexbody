...
table <<'EOF'
tests/form.loft|FORM OK|/tmp/hexbody_form.log|headings|[1/8] the 12 headings, the exact rotation and reflection (S0/S1) ...
tests/wall.loft|WALL OK|/tmp/hexbody_wall.log|walls|[2/8] the 24-direction wall: edge table, foxel write, mesh evaluation ...
tests/box.loft|BOX OK|/tmp/hexbody_box.log|boxes|[3/8] the box in 12 directions, thin wall and thick wall ...
tests/census.loft|CENSUS OK|/tmp/hexbody_census.log|census|[4/8] the census, grown by level: law F decided at levels 1-3 ...
tests/text.loft|TEXT OK|/tmp/hexbody_text.log|text|[5/8] the canonical text: write(read(T)) = T byte-for-byte ...
tests/house.loft|HOUSE OK|/tmp/hexbody_house.log|house|[6/8] floor/walls/openings/roof at all 12 orientations ...
tests/surface.loft|SURFACE OK|/tmp/hexbody_surface.log|wall surface|[7/8] the wall surface by averaging: exact direction, exact bands ...
tests/trip.loft|TRIP OK|/tmp/hexbody_trip.log|round trip|[8/8] rt_trip: write(rebuild(draw(read(T)))) == T over corpus a1+a2 ...
EOF
