...
table <<'EOF'
tests/form.loft|FORM OK|/tmp/hexbody_form.log|headings|[1/10] the 12 headings, the exact rotation and reflection (S0/S1) ...
tests/wall.loft|WALL OK|/tmp/hexbody_wall.log|walls|[2/10] the 24-direction wall: edge table, foxel write, mesh evaluation ...
tests/box.loft|BOX OK|/tmp/hexbody_box.log|boxes|[3/10] the box in 12 directions, thin wall and thick wall ...
tests/census.loft|CENSUS OK|/tmp/hexbody_census.log|census|[4/10] the census, grown by level: law F decided at levels 1-3 ...
tests/text.loft|TEXT OK|/tmp/hexbody_text.log|text|[5/10] the canonical text: write(read(T)) = T byte-for-byte ...
tests/house.loft|HOUSE OK|/tmp/hexbody_house.log|house|[6/10] floor/walls/openings/roof at all 12 orientations ...
tests/surface.loft|SURFACE OK|/tmp/hexbody_surface.log|wall surface|[7/10] the wall surface by averaging: exact direction, exact bands ...
tests/arc.loft|ARC OK|/tmp/hexbody_arc.log|arcs|[8/10] the round tower + the doored tower: centre exact, radius a shell, a door is one arc ...
tests/combine.loft|COMBINE OK|/tmp/hexbody_combine.log|combine|[9/10] two stencils adjacent: union-then-cut is order-free, the shared edge fuses (A8) ...
tests/trip.loft|TRIP OK|/tmp/hexbody_trip.log|round trip|[10/10] rt_trip: write(rebuild(draw(read(T)))) == T over corpus a1+a2 ...
EOF
