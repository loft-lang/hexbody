...
| **X60** | **a stencil CAN carry a wall in an in-between direction — the footprint and the round trip are untouched** — `OD-13`'s load-bearing half, probed before any grammar work. An embedded wall is a **material on INTERIOR edges** (both cells in the footprint), which is geometrically distinguishable from a boundary edge with no extra tagging. Measured on a 127-cell hexagon: the footprint stays **127 cells**, `rebuild` returns the **authored text**, the run is **wholly interior**, and its chain is **well-formed — 2 ends, 0 branches** (the gated `X32` measure), for every in-between direction that fits. Control: the same wall drawn as a **gap** (5 cells removed) **breaks** the round trip, so orthogonality is a result and not a restatement. **`OD-13` is NOT closed by this** — `rebuild` still does not *read* that chain, so an embedded wall is **silently dropped and `rt_trip` does not notice**, which is exactly the site the design flagged as the dangerous one. ⚠ **Two things observed and NOT explained:** only **4 of 12** in-between directions admit a whole period wholly inside the stencil (widening the anchor search ±14 → ±26 and doubling the stencil both found the same four — structural, not a search artefact); and **three separate reader instruments for recovering `(d, anchor, p)` were tried and all three were wrong**, so nothing about recovery is asserted | **T1** ✅ (orthogonality) / **OPEN** (recovery, coverage) | **`tests/embed.loft`** — control: an embedded wall drawn as removed cells must break the round trip |
