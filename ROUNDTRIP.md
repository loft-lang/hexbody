...
| **X56** | **domain B's cost is a THREE-CLASS period table — and today's in-between vector is dominated** — `rt_census_b`, the census `D`'s closure (`X3`) left open. Representability was never the question; **cost** was. Measured over all 24 with the gated `wall_run_len`/`wall_min_p`: **6** directions at period `√3` wu and **6** at period `1` wu, all **exact** in angle; **12** at period `√21` wu = **3.969 m**, all `4.1066°` off nominal (`X29` re-confirmed, 12 exact + 12 off, 0 neither). **It corrected a design doc:** DESIGN §10.9's ladder listed the period as `√N/3`; the gated primitive says `√N`, agreeing with §10.10's `3.969 m` — a clean **3×** factor, the signature of a counter bug, here in the table rather than the code. The *ratios* were unaffected, so **`X31`'s ladder conclusion stands and is now T1**: `(5,3)`/`N=21` is **dominated** — `(3,1)`/`N=7` matches its `4.1066°` error at a **42% shorter** period, and `(4,2)`/`N=13` beats it on **both** axes (`1.1021°`, 21% shorter). Changing it remains a **live proposal**, not a decision (`I-EXTEND`) | **T1** ✅ | **`tests/censusb.loft`** — controls: `(15,7)` must be a genuine **trade** (better angle, longer period) or "dominated" is empty; the measured period must **not** equal `√N/3` |
...
> `X24`–`X56`. Of those, **eight were re-measured *here*** — `X1`, `X2`, `X20`, `X24`, `X25` by
